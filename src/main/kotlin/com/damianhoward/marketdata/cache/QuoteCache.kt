 ...
 * - **Negative cache:** a symbol the provider reports it does not carry ([UnknownSymbol]), with no
 *   last-good mark, is remembered for [negativeTtl], so repeated requests for the same unknown
 *   ticker don't each hit the provider. The set is LRU-bounded at [maxNegativeEntries] so free-text
 *   symbol entry can't grow it without bound. A symbol that later resolves clears its mark.
 *   A provider *failure* is never remembered this way: an outage is not evidence about a symbol,
 *   and treating it as such would keep a valid ticker first requested mid-outage suppressed for
 *   [negativeTtl] after the provider came back.
...
class QuoteCache(
     ...
     * caller polling this never has to handle the failure itself. A symbol recently reported unknown,
     * with no last-good mark, short-circuits to null without touching the provider (negative cache);
     * a symbol whose last attempt merely failed is retried.
     */
    fun refresh(symbol: String): CachedQuote? {
        ...
        return try {
            ...
            clearMiss(symbol)
            cached
        } catch (e: UnknownSymbol) {
            if (lastGood == null) recordMiss(symbol)
            log.warn("Quote refresh found no instrument for {}; serving {}", symbol, served(lastGood), e)
            lastGood
        } catch (e: QuoteUnavailable) {
            log.warn("Quote refresh failed for {}; serving {}", symbol, served(lastGood), e)
            lastGood
        }
    }

    private fun served(lastGood: CachedQuote?): String = if (lastGood == null) "nothing yet" else "last-good"
    ...
    companion object {
        /** How long a symbol the provider does not carry is remembered before it is tried again. */
